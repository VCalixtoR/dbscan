use crate::configuration::{ ClusterGroup, IndexType, KmeansConfig, Point, PointVector };
use crate::utils::euclidian_distance;

pub fn calc_centroid_by_mean(config: &KmeansConfig, centroid_points: &mut PointVector, cluster_group: &ClusterGroup) -> () {

    let mut cluster_sum: Point;

    for cluster_pos in 0..cluster_group.clusters.len() {
        // sum all values from a cluster
        cluster_sum = vec![0.0; config.database_points[0].len() as usize];
        for core_pos in 0..cluster_group.clusters[cluster_pos].core_indexes.len() {
            ...
            for value_pos in 0..config.database_points[index].len() {
                cluster_sum[value_pos] += config.database_points[index][value_pos];
            }
        }
...
pub fn kmeans_clusterization(config: KmeansConfig) -> ClusterGroup {
    
    let mut centroid_points: PointVector = config.centroid_points.clone();
    ...
    while cluster_group_has_changed {
        ...
        // foreach point
        for point_index in 0..config.database_points.len() {
            ...
            for centroid_index in 0..centroid_points.len() {
                tmp_float = euclidian_distance(&centroid_points[centroid_index], &config.database_points[point_index]);
        ...
        if cluster_group_has_changed {
            ...
            calc_centroid_by_mean(&config, &mut centroid_points, &cluster_group);
        }
